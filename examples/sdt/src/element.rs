...
use idp2p_common::utils::to_hex_str;
    ...
    pub fn to_element(&self) -> SdtElement {
        ...
        if let SdtClaim::Element(map) = &self {
            for (k, v) in map {
                match v {
                    SdtClaim::Value(val) => {
                        let salt = to_hex_str(&wasi::random::random::get_random_bytes(32));
                        element.add_value(k, &salt, val.to_owned());
                    }
                    SdtClaim::Element(_) => {
...
#[derive(PartialEq, Debug, Clone, Serialize, Deserialize)]
pub struct SdtElement(BTreeMap<String, SdtElementKind>);
...
impl SdtElement {
    pub fn new() -> Self {
        let map: BTreeMap<String, SdtElementKind> = BTreeMap::new();
        Self(map)
    }
    ...
    pub fn add_value(&mut self, key: &str, salt: &str, val: SdtValueKind) -> &mut Self {
        self.0.insert(
            key.to_string(),
            SdtElementKind::Value(SdtValue::new(salt, val)),
        );
        self
    }
    ...
    pub fn add_proof(&mut self, key: &str, proof: &str) -> &mut Self {
        self.0
            .insert(key.to_string(), SdtElementKind::Proof(proof.to_string()));
        self
    }

    pub fn add_str_value(&mut self, key: &str, salt: &str, val: &str) -> &mut Self {
        self.add_value(key, salt, SdtValueKind::String(val.to_string()))
    }

    pub fn add_number_value(&mut self, key: &str, salt: &str, val: i64) -> &mut Self {
        self.add_value(key, salt, SdtValueKind::Number(Number::from(val)))
    }

    pub fn add_bool_value(&mut self, key: &str, salt: &str, val: bool) -> &mut Self {
        self.add_value(key, salt, SdtValueKind::Bool(val))
    }

    pub fn add_null_value(&mut self, key: &str, salt: &str) -> &mut Self {
        self.add_value(key, salt, SdtValueKind::Null)
    }
    ...
    pub fn to_claim(&self) -> SdtClaim {
        let mut map: BTreeMap<String, SdtClaim> = BTreeMap::new();
        for (k, v) in &self.0 {
            match v {
                SdtElementKind::Value(val) => {
                    map.insert(k.to_string(), SdtClaim::Value(val.value.to_owned()));
                }
                SdtElementKind::Element(inner) => {
                    map.insert(k.to_string(), inner.to_claim());
                }
                _ => {}
            }
        }
    ...
    pub fn select(&mut self, query: &str) -> anyhow::Result<()> {
        ...
        while let Some((path, element)) = stack.pop() {
            let mut path_keys: BTreeMap<String, String> = BTreeMap::new();
...
mod tests {
    ...
    #[test]
    fn test_full_proof() -> anyhow::Result<()> {
        let result_str = r#"
        {
            "personal": {
              "name": {
