...
impl<S: KvStore> IdNetworkEventLoop<S> {
    pub fn new(
        ...
        let swarm = create_swarm(users.lock().unwrap().get_current_user().port)?;
        Ok((
                            ...
                            self.swarm
                                ...
                                .send_response(
                                    ...
                                    IdResponseKind::GetResult {
                                        username: users.current.clone(),
                                        id: users.get_current_user().id.unwrap().to_owned(),
                                    },
                                )
                                .unwrap();
                        }
