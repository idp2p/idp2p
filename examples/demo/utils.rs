...
use idp2p_id::inception::IdInception;
use idp2p_p2p::handler::{PersistedId, PersistedIdInception};
use libp2p::PeerId;
use rand::rngs::OsRng;
...
pub fn generate_id(mediator: &PeerId) -> Result<PersistedId> {
    ...
    let inception = IdInception::generate(&signing_key.to_bytes(), &mediator.to_string())?;
